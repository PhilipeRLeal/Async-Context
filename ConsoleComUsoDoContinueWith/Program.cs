...
public class Program
{
    public static void Main()
    {
        ...
        // atribuir true ou false aqui não altera o resultado:
        var task = CustomObterJsonAsync(uri, true);
        ...
        Console.WriteLine("Notar que o ContinueWith nunca é executado, não importa quanto aguardemos. Tanto que somente vemos a thread 1 sendo utilizada.");
        Console.ReadLine();
    ...
    {
        Console.WriteLine("Executando alguma outra coisa na main thread, enquanto aguardamos o resultado do 'CustomObterJsonAsync' ");
        ReportThread();
        Thread.Sleep(1000);
    ...
    private static async Task<string[]> CustomObterJsonAsync(Uri uri, bool configureAwait = true)
    {
        ...
        using ( var client = new WebClient() )
        {
            ...
            var dadosProcessadosTask = task.ContinueWith(t =>
            {
                ...
                Console.WriteLine($"Thread Id do ContinueWith: {Thread.CurrentThread.ManagedThreadId} com configureAwait = {configureAwait}.");
            ...
            var dadosProcessados = await dadosProcessadosTask;
